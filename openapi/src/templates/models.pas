...
{% macro from_json(base_type, is_list_type, is_reference_type, is_enum_type, key) %}
  ...
  {{ macros::type_name(base_type=base_type, is_list_type=is_list_type, is_reference_type=is_reference_type, is_enum_type=is_enum_type) }}.FromJsonRaw(vRoot.GetValue<TJSONObject>({{key}}))
...
begin
  {%- for property in classType.properties %}
  F{{property.name}} := {{ self::from_json(base_type=property.type_.name, is_list_type=property.is_list_type, is_reference_type=property.type_.is_class, is_enum_type=property.type_.is_enum, key="cn" ~ classType.name ~ property.key ~ "Key") }};
  {%- endfor%}
end;
